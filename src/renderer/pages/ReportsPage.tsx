...
import type { ExportResult, MetricName, SessionDetail, SessionSummary } from "@shared/types";
...
function normalizeVisibleRange(range: VisibleRange | null, sampleCount: number): VisibleRange | null {
  if (sampleCount <= 0) {
    return null;
  }
  ...
  return {
    ...
    endIndex: Math.max(0, Math.min(maxIndex, Math.max(range.startIndex, range.endIndex)))
  };
}
...
function calculateMetricStats(
  ...
  const normalizedRange = normalizeVisibleRange(range, sessionDetail?.samples.length ?? 0);
...
export function ReportsPage() {
  ...
  const [selectedScreenshotPosition, setSelectedScreenshotPosition] = useState(-1);
  const [selectedScreenshotUrl, setSelectedScreenshotUrl] = useState("");
  const [isScreenshotLoading, setIsScreenshotLoading] = useState(false);
  const [loadChartRange, setLoadChartRange] = useState<VisibleRange | null>(null);

  const controlsDisabled = refreshing || busyAction !== null || renameDialogOpen || deleteDialogOpen;
  const latestMetrics = sessionDetail?.samples[sessionDetail.samples.length - 1]?.metrics;
  const chartSyncGroup = sessionDetail ? `report-metric-charts-${sessionDetail.id}` : undefined;
  const screenshotSampleIndexes = useMemo(() => {
    if (!sessionDetail) {
      return [] as number[];
    }

    return sessionDetail.samples.reduce<number[]>((result, sample, index) => {
  ...
  const selectedScreenshotSampleIndex =
    selectedScreenshotPosition >= 0 ? screenshotSampleIndexes[selectedScreenshotPosition] ?? null : null;
  ...
  const normalizedLoadChartRange = useMemo(
    () => normalizeVisibleRange(loadChartRange, sessionDetail?.samples.length ?? 0),
    [loadChartRange, sessionDetail]
  );
  const loadChartStats = useMemo(
    () => [
      {
        label: sessionDetail?.config.cpuMode === "normalized" ? "App CPU Norm.(%)" : "App CPU(%)",
        stats: calculateMetricStats(sessionDetail, normalizedLoadChartRange, "cpu")
      },
      {
        label: sessionDetail?.config.cpuMode === "normalized" ? "Total CPU Norm.(%)" : "Total CPU(%)",
        stats: calculateMetricStats(sessionDetail, normalizedLoadChartRange, "cpuTotal")
      },
      {
        label: "GPU(%)",
        stats: calculateMetricStats(sessionDetail, normalizedLoadChartRange, "gpu")
      }
    ],
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const detail = await window.lyPerf.getSession(selectedSessionId);
        if (cancelled) {
          return;
        }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const dataUrl = await window.lyPerf.readScreenshotDataUrl(screenshotPath);
        if (!disposed) {
  ...
  async function handleExport(format: "html" | "xlsx"): Promise<void> {
    ...
    try {
      const result = await window.lyPerf.exportSession(selectedSessionId, format);
  ...
  async function handleRename(nextName: string): Promise<void> {
    ...
    try {
      const renamed = await window.lyPerf.renameSession(sessionDetail.id, normalizedName);
      ...
      setRenameDialogError(getErrorMessage(error, "重命名历史会话失败。"));
    } finally {
      setBusyAction(null);
    }
  }
  ...
  function handleChartSampleFocus(sampleIndex: number): void {
    ...
    const nextPosition = findNearestIndex(screenshotSampleIndexes, sampleIndex);
    setSelectedScreenshotPosition((current) => (current === nextPosition ? current : nextPosition));
  }
    ...
    <section className={styles.page}>
      <aside className={styles.sidebar}>
        <div className={styles.headerRow}>
          <h3>历史会话</h3>
          <button type="button" disabled={controlsDisabled} onClick={() => void reloadSessions()}>
            {refreshing ? "刷新中..." : "刷新"}
          </button>
        </div>

        <ul className={styles.sessionList}>
          {sessions.length === 0 ? <li className={styles.empty}>暂无历史会话</li> : null}
          ...
          {sessions.map((session) => (
            <li key={session.id}>
              <button
                ...
                className={selectedSessionId === session.id ? styles.sessionActive : styles.sessionBtn}
                onClick={() => setSelectedSessionId(session.id)}
              >
                <div className={styles.sessionTitle}>{session.displayName}</div>
                {session.displayName !== session.packageName ? (
                  <div className={styles.sessionMeta}>包名: {session.packageName}</div>
                ) : null}
                <div className={styles.sessionMeta}>{session.serial}</div>
                <div className={styles.sessionMeta}>开始: {formatDate(session.startedAt)}</div>
                <div className={styles.sessionMeta}>样本: {session.sampleCount}</div>
              </button>
            </li>
          ))}
        </ul>
      ...
      <div className={styles.content}>
        {feedback ? (
          <p className={feedback.type === "error" ? styles.errorMessage : styles.statusMessage}>{feedback.text}</p>
        ) : null}
          ...
          <>
            <header className={styles.detailHeader}>
              <div>
                ...
                <p>
                  包名 {sessionDetail.packageName} | 设备 {sessionDetail.deviceInfo.brand} {sessionDetail.deviceInfo.model} ({sessionDetail.serial})
                </p>
              </div>
              <div className={styles.detailActions}>
                <div className={styles.exportButtons}>
                  <button type="button" disabled={controlsDisabled} onClick={openRenameDialog}>
                    重命名
                  </button>
                  <button
                    type="button"
            ...
            {exportResult ? (
              <p className={styles.exportResult}>
                已导出 {exportResult.format.toUpperCase()} 报告: {exportResult.outputPath}
              </p>
            ) : null}
            ...
            <div className={styles.summaryGrid}>
              <div>
                <span>开始时间</span>
                <strong>{formatDate(sessionDetail.startedAt)}</strong>
              </div>
              <div>
                <span>结束时间</span>
                <strong>{formatDate(sessionDetail.endedAt)}</strong>
              </div>
              <div>
                <span>采样总数</span>
              ...
              <div>
                <span>截图策略</span>
                <strong>{sessionDetail.config.screenshotEnabled ? "开启" : "关闭"}</strong>
              </div>
              <div>
                <span>FPS来源</span>
                <strong>{sessionDetail.config.fpsMode === "gfxinfo" ? "gfxinfo" : "SurfaceFlinger"}</strong>
              </div>
              <div>
                <span>CPU口径</span>
                <strong>
              ...
              <div>
                <span>总流量</span>
                <strong>{formatTraffic(latestMetrics?.networkTotal?.value)}</strong>
              </div>
            </div>
            <div className={styles.detailBody}>
              <section className={styles.chartSection}>
                <div className={styles.sectionHeader}>
                  <h4>历史趋势图</h4>
                  <span>保留完整采样序列，布局与实时监控一致，缩放区域联动。</span>
                </div>
                ...
                {sessionDetail.samples.length > 0 ? (
                    ...
                    <MetricChart
                      ...
                      onVisibleRangeChange={(startIndex, endIndex) => {
                        setLoadChartRange((current) =>
                          current && current.startIndex === startIndex && current.endIndex === endIndex
                            ? current
                            : { startIndex, endIndex }
                        );
                      }}
                    ...
                    <div className={styles.metricStatsPanel}>
                      ...
                      <div className={styles.metricStatsGrid}>
                        {loadChartStats.map((item) => (
                          <div key={item.label} className={styles.metricStatsCard}>
                            <strong>{item.label}</strong>
                            <span>最大值 {formatStatValue(item.stats?.max ?? null)}</span>
                            <span>最小值 {formatStatValue(item.stats?.min ?? null)}</span>
                            <span>平均值 {formatStatValue(item.stats?.average ?? null)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <p className={styles.empty}>该历史会话暂无采样数据，暂时无法绘制趋势图。</p>
                )}
              </section>
                  ...
                  <>
                    <div className={styles.previewToolbar}>
                      <div className={styles.previewMeta}>
                        <strong>{formatDate(selectedScreenshotSample.timestamp)}</strong>
                        <span>
                          第 {selectedScreenshotPosition + 1} / {screenshotSampleIndexes.length} 张截图
                        </span>
                      </div>
                    ...
                    {selectedScreenshotUrl ? (
                      ...
                      <p className={styles.empty}>截图加载失败，文件可能已不存在或不可访问。</p>
                    )}
                  </>
                ) : (
                  <p className={styles.empty}>该历史会话没有可预览截图。开启截图采样后再开始监控即可保留历史截图。</p>
                )}
              </section>
            </div>
          </>
        ) : (
          <p className={styles.empty}>请选择左侧会话查看详情和导出。</p>
        )}
