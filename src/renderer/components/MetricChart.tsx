...
function getVisibleSampleRange(chart: echarts.ECharts, sampleCount: number): { startIndex: number; endIndex: number } {
  if (sampleCount <= 0) {
  ...
  const startIndex = Math.max(0, Math.min(maxIndex, Math.floor((startPercent / 100) * maxIndex)));
...
export function MetricChart(props: MetricChartProps) {
  ...
  const option = useMemo(() => {
    const labels = props.samples.map((sample) => formatTime(sample.timestamp));
    ...
    return {
      ...
      series: props.series.map((item) => ({
        ...
        data: props.samples.map((sample) => sample.metrics[item.key]?.value ?? null)
      }))
    };
  ...
  useEffect(() => {
    ...
    const handleAxisPointerUpdate = (event: unknown) => {
      const axisInfo = (event as { axesInfo?: Array<{ value?: number | string }> }).axesInfo?.[0];
      const axisValue = axisInfo?.value;
      const sampleIndex = typeof axisValue === "number" ? axisValue : Number(axisValue);

      if (!Number.isInteger(sampleIndex) || sampleIndex < 0 || sampleIndex >= props.samples.length) {
        return;
      }
  ...
  }, [option, props.onSampleFocus, props.onVisibleRangeChange, props.samples.length, props.syncGroup]);
