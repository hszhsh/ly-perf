    ...
    <div className={styles.shell}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Android App Performance Monitor</p>
        </div>
        <nav className={styles.tabs}>
          <button
            className={activeTab === "monitor" ? styles.tabActive : styles.tab}
          ...
          <button
            ...
            title={reportsDisabled ? "实时监控启动中或进行中时，不能切换到历史报告。" : undefined}
            type="button"
          >
            历史报告
          </button>
      ...
      <main className={styles.main}>
        {activeTab === "monitor" ? <MonitorPage onMonitorBusyChange={setIsMonitorBusy} /> : <ReportsPage />}
      </main>
    </div>
  );
}
