...
const GPU_MODEL_HINT =
    /\b(adreno|mali|powervr|immortalis|xclipse|vivante|tegra|apple|geforce|radeon)\b/i;
const GPU_STATS_NOISE =
    /\b(missed frame count|frame count|frame drop|latency|jank|present time|vsync)\b/i;
...
function extractGpuModelFromLine(line: string): string | null {
    ...
    const openGlMatch = normalized.match(
        /OpenGL(?:\s+ES)?(?:\s+\d+(?:\.\d+)*)?\s*[:=-]\s*(.+)$/i
    );
    if (openGlMatch?.[1]) {
...
function parseGpuModel(gpuText: string): string {
    ...
    const lines = gpuText.split(/\r?\n/).map(cleanGpuLine).filter(Boolean);
...
function parseOpenGlVersion(
    gpuText: string,
    openGlVersionProp: string
): string {
...
function parseVulkanVersion(
    gpuDumpText: string,
    vulkanVersionProp: string
): string {
...
export class DeviceService {
    ...
    async getDeviceInfo(serial: string): Promise<DeviceInfo> {
        ...
        return {
            ...
            openGlVersion: parseOpenGlVersion(
                [gpuText, gpuPropText, gpuDumpText].filter(Boolean).join("\n"),
                openGlVersionProp
            ),
            vulkanVersion: parseVulkanVersion(gpuDumpText, vulkanVersionProp),
            cpuModel: parseCpuModel(cpuInfoText, [
                socModel,
                boardPlatform,
                hardwareName,
                bootHardware,
