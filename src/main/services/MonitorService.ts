...
export class MonitorService {
  ...
  constructor(
    ...
    private readonly onSample: (sessionId: string, sample: MonitorSample) => void
  ) {}
  ...
  async start(config: MonitorConfig): Promise<MonitorState> {
    ...
    const normalized: MonitorConfig = {
      ...
      screenshotIntervalMs: normalizeInterval(config.screenshotIntervalMs, 2000)
    };

    const deviceInfo = await this.deviceService.getDeviceInfo(normalized.serial);
    ...
    await this.tryLaunchTargetApp(normalized.serial, normalized.packageName);
  ...
  async stop(): Promise<MonitorState> {
    ...
    await this.sessionStore.saveSession(active.session);
  ...
  getCapabilityReport(): MetricCapabilityReport | null {
    ...
    return this.collector.getCapabilityReport(active.session.serial, active.session.packageName);
  }
  ...
  private captureScreenshotIfNeeded(active: ActiveMonitor): void {
    ...
    const screenshotDir = this.sessionStore.getScreenshotDir(active.session.id);
  ...
  private consumePendingScreenshotPath(active: ActiveMonitor): string | undefined {
    const screenshotPath = active.pendingScreenshotPath;
    active.pendingScreenshotPath = undefined;
    return screenshotPath;
  }

  private async tryLaunchTargetApp(serial: string, packageName: string): Promise<void> {
    try {
