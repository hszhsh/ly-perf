...
export class ReportService {
  constructor(private readonly store: SessionStore, private readonly appRoot: string) {}

  async exportSession(sessionId: string, format: "html" | "xlsx"): Promise<ExportResult> {
    if (format === "html") {
  ...
  private async exportHtml(sessionId: string): Promise<ExportResult> {
    ...
    const outputDir = path.join(this.store.getDataDir(), "exports", sessionId, "html");

    await fs.rm(outputDir, { recursive: true, force: true });
    await fs.mkdir(outputDir, { recursive: true });

    const reportSession = await this.copyScreenshotsForReport(session, outputDir);

    const echartsSource = path.join(this.appRoot, "node_modules", "echarts", "dist", "echarts.min.js");
    const echartsTarget = path.join(outputDir, "echarts.min.js");

    await fs.copyFile(echartsSource, echartsTarget);
    await fs.writeFile(path.join(outputDir, "report.json"), JSON.stringify(reportSession, null, 2), "utf8");
    await fs.writeFile(path.join(outputDir, "index.html"), this.buildHtmlTemplate(), "utf8");
  ...
  private async exportXlsx(sessionId: string): Promise<ExportResult> {
    const session = await this.store.getSession(sessionId);
    const outputDir = path.join(this.store.getDataDir(), "exports", sessionId, "xlsx");
  ...
  private async copyScreenshotsForReport(session: SessionDetail, outputDir: string): Promise<SessionDetail> {
      ...
      session.samples.map(async (sample, index) => {
        ...
        if (!copiedPathMap.has(sample.screenshotPath)) {
          ...
          try {
            ...
            copiedPathMap.set(sample.screenshotPath, toWebPath(path.join("screenshots", fileName)));
          } catch {
            copiedPathMap.set(sample.screenshotPath, "");
          }
        }

        const copied = copiedPathMap.get(sample.screenshotPath) || undefined;
