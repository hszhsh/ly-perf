...
export function registerIpcHandlers(deps: RegisterIpcDependencies): void {
  ...
  ipcMain.handle(IPC_CHANNELS.listDevices, async () => deps.deviceService.listDevices());

  ipcMain.handle(IPC_CHANNELS.getDeviceInfo, async (_event, serial: string) =>
    deps.deviceService.getDeviceInfo(serial)
  );

  ipcMain.handle(IPC_CHANNELS.listInstalledApps, async (_event, serial: string, keyword?: string) =>
    deps.deviceService.listInstalledApps(serial, keyword)
  );

  ipcMain.handle(IPC_CHANNELS.readScreenshotDataUrl, async (_event, filePath: string) => {
  ...
  ipcMain.handle(IPC_CHANNELS.startMonitor, async (_event, config: MonitorConfig) =>
    deps.monitorService.start(config)
  );

  ipcMain.handle(IPC_CHANNELS.stopMonitor, async () => deps.monitorService.stop());

  ipcMain.handle(IPC_CHANNELS.getMonitorState, async () => deps.monitorService.getState());

  ipcMain.handle(IPC_CHANNELS.getMonitorCapabilityReport, async () => deps.monitorService.getCapabilityReport());

  ipcMain.handle(IPC_CHANNELS.listSessions, async () => deps.sessionStore.listSessions());

  ipcMain.handle(IPC_CHANNELS.getSession, async (_event, sessionId: string) =>
    deps.sessionStore.getSession(sessionId)
  );

  ipcMain.handle(IPC_CHANNELS.renameSession, async (_event, sessionId: string, displayName: string) =>
    deps.sessionStore.renameSession(sessionId, displayName)
  );

  ipcMain.handle(IPC_CHANNELS.deleteSession, async (_event, sessionId: string) =>
    deps.sessionStore.deleteSession(sessionId)
  );

  ipcMain.handle(IPC_CHANNELS.exportSession, async (_event, sessionId: string, format: "html" | "xlsx") =>
